...
export const useGetInventoryListCoreQuery = ({
    ...
    inventoryQueryparams = Object.keys(queryParams)
      ...
      .reduce<InventoryQueryParams>((acc, key) => {
        if (queryParams[key as keyof InventoryQueryParams]) {
          acc = {
            ...acc,
            [key as keyof InventoryQueryParams]: queryParams[key as keyof InventoryQueryParams],
          };
        }
        return acc;
  ...
  return useQuery({
    ...
    select: (data) => {
      const { result, pageCurrent, pageSize, totalItems, totalPages } = data;
      return {
        list: result,
        pageCurrent,
        pageSize,
...
export const useGetInventoryDetailCoreQuery = ({ recId, enabled = false }: { recId?: number; enabled?: boolean }) => {
  return useQuery({
    queryKey: [queryCore.GET_INVENTORY_DETAIL, recId],
    queryFn: () => inventoryAPIs.getDetail(recId),
