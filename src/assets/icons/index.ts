import IconAccount from "./IconAccount";
import IconArrowLeftCircle from "./IconArrowLeftCircle";
import IconArrowRightCircle from "./IconArrowRightCircle";
import IconCalendar from "./IconCalendar";
import IconCalendarCheck from "./IconCalendarCheck";
import IconCalendarPlus from "./IconCalendarPlus";
import IconChevronDown from "./IconChevronDown";
import IconChevronRight from "./IconChevronRight";
import IconChevronLeft from "./IconChevronLeft";
import IconChevronUp from "./IconChevronUp";
import IconClipboard from "./IconClipboard";
import IconScrollText from "./IconScrollText";
import IconFlagVN from "./IconFlagVN";
import IconFlagEN from "./IconFlagEN";
import IconHeart from "./IconHeart";
import IconHotel from "./IconHotel";
import IconMail from "./IconMail";
import IconMapPin from "./IconMapPin";
import IconStar from "./IconStar";
import IconUser from "./IconUser";
import IconSupport from "./IconSupport";
import IconShare from "./IconShare";
import IconShippingCart from "./IconShoppingCart";
import IconSearch from "./IconSearch";
import IconPlane from "./IconPlane";
import IconMeal from "./IconMeal";
import IconTicketPercent from "./IconTicketPercent";
import IconCheck from "./IconCheck";
import IconBus from "./IconBus";
import IconBuilding from "./IconBuilding";
import IconStarOutline from "./IconStarOutline";
import IconUtensilOutline from "./IconUtensilOutline";
import IconPen from "./IconPen";
import IconRecieptText from "./IconReceiptText";
import IconLogout from "./IconLogout";
import IconCitrus from "./IconCitrus";
import IconCaravan from "./IconCaravan";
import IconCloudMoon from "./IconCloudMoon";
import IconCookie from "./IconCookie";
import IconCrown from "./IconCrown";
import IconDumbellOutline from "./IconUtensilOutline";
import IconFlame from "./IconFlame";
import IconFlower from "./IconFlower";
import IconShield from "./IconShield";
import IconShieldCheck from "./IconShieldCheck";
import IconShieldOff from "./IconShieldOff";
import IconShieldPlus from "./IconShieldPlus";
import IconSparklesOutline from "./IconSparklesOutline";
import IconBellRing from "./IconBellRing";
import IconCar from "./IconCar";
import IconCheckCircle from "./IconCheckCircle";
import IconDollarSignCircle from "./IconDollarSignCircle";
import IconClock from "./IconClock";
import IconCloudSun from "./IconCloudSun";
import IconFlagTriangleRight from "./IconFlagTriangleRight";
import IconHouse from "./IconHouse";
import IconImage from "./IconImage";
import IconPlaneLading from "./IconPlaneLading";
import IconPlaneTakeOff from "./IconPlaneTakeOff";
import IconTag from "./IconTag";
import IconTags from "./IconTags";
import IconTicket from "./IconTicket";
import IconTriangleAlert from "./IconTriangleAlert";
import IconHelpCircle from "./IconHelpCircle";
import IconMinusCircle from "./IconMinusCircle";
import IconAlertCircle from "./IconAlertCircle";
import IconXCircle from "./IconXCircle";
import IconSend from "./IconSend";
import IconUserPlus from "./IconUserPlus";
import IconUserCheck from "./IconUserCheck";
import IconUsers from "./IconUsers";
import IconBan from "./IconBan";
import IconBookMark from "./IconBookMark";
import IconCalendarDays from "./IconCalendarDays";
import IconLayoutGrid from "./IconLayoutGrid";
import IconMapPinned from "./IconMapPinned";
import IconMessageSquareMore from "./IconMessageSqureMore";
import IconPackageCheck from "./IconPackageCheck";
import IconPartyPopper from "./IconPartyPopper";
import IconKeyRound from "./IconKeyRound";
import IconImages from "./IconImages";
import IconAlbum from "./IconAlbum";
import IconEarth from "./IconEarth";
import IconClipboardList from "./IconClipboardList";
import IconTwitter from "./IconTwitter";
import IconLink from "./IconLink";
import IconFlameKining from "./IconFlameKining";
import IconFacebook from "./IconFacebook";
import IconFiles from "./IconFiles";
import IconFile from "./IconFile";
import IconPlanet from "./IconPlanet";
import IconHotDealOne from "./label/IconHotDealOne";
import IconHotDealTwo from "./label/IconHotDealTwo";
import IconHotSale from "./label/IconHotSale";
import IconHotSaleTwo from "./label/IconHotSaleTwo";
import IconBestDeal from "./label/IconBestDeal";
import IconBigDeal from "./label/IconBigDeal";
import IconDiscount from "./label/IconDiscount";
export {
  IconHotDealOne,
  IconHotDealTwo,
  IconHotSale,
  IconHotSaleTwo,
  IconBestDeal,
  IconBigDeal,
  IconDiscount,
  IconFile,
  IconFiles,
  IconTwitter,
  IconLink,
  IconFlameKining,
  IconFacebook,
  IconAccount,
  IconArrowLeftCircle,
  IconArrowRightCircle,
  IconCalendar,
  IconCalendarCheck,
  IconCalendarPlus,
  IconChevronDown,
  IconChevronRight,
  IconChevronLeft,
  IconChevronUp,
  IconClipboard,
  IconScrollText,
  IconFlagVN,
  IconFlagEN,
  IconHeart,
  IconHotel,
  IconMail,
  IconMapPin,
  IconStar,
  IconUser,
  IconSupport,
  IconShare,
  IconShippingCart,
  IconSearch,
  IconPlane,
  IconMeal,
  IconTicketPercent,
  IconCheck,
  IconBus,
  IconBuilding,
  IconStarOutline,
  IconUtensilOutline,
  IconPen,
  IconRecieptText,
  IconLogout,
  IconCitrus,
  IconCaravan,
  IconCloudMoon,
  IconCookie,
  IconCrown,
  IconDumbellOutline,
  IconFlame,
  IconFlower,
  IconShield,
  IconShieldCheck,
  IconShieldOff,
  IconShieldPlus,
  IconSparklesOutline,
  IconBellRing,
  IconCar,
  IconCheckCircle,
  IconDollarSignCircle,
  IconClock,
  IconCloudSun,
  IconFlagTriangleRight,
  IconHouse,
  IconImage,
  IconPlaneLading,
  IconPlaneTakeOff,
  IconTag,
  IconTags,
  IconTicket,
  IconTriangleAlert,
  IconHelpCircle,
  IconMinusCircle,
  IconAlertCircle,
  IconXCircle,
  IconSend,
  IconUserPlus,
  IconUserCheck,
  IconUsers,
  IconBan,
  IconBookMark,
  IconCalendarDays,
  IconLayoutGrid,
  IconMapPinned,
  IconMessageSquareMore,
  IconPackageCheck,
  IconPartyPopper,
  IconKeyRound,
  IconImages,
  IconAlbum,
  IconEarth,
  IconClipboardList,
  IconPlanet,
};
