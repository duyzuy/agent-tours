...
const useUpdateRolePermission = (localUser_RolePermissionKey: string) => {
  ...
  const { mutate: makeUpdateRolePermissions } = useUpdateRolePermissionsMutation(localUser_RolePermissionKey);

  const message = useMessage();
  const onUpdateRolePermissions = (payload: TRolePermissionPayload, cb?: () => void) => {
      ...
      .then((data) => {
        makeUpdateRolePermissions(data, {
          onSuccess: (response, variable) => {
            ...
            message.success(`Cập nhật nhóm chức năng ${variable.localUser_RolePermissionValue} thành công.`);
