...
import { useCreateDestinationMutation, useUpdateDestinationMutation } from "@/mutations/managements/destination";
import useMessage from "@/hooks/useMessage";
import { useQueryClient } from "@tanstack/react-query";
import { queryMisc } from "@/queries/var";
import { Status } from "@/models/common.interface";
export type TDestinationErrorsField = Partial<
  Record<keyof Pick<IDestinationPayload, "codeKey" | "codeName" | "listStateProvince" | "status">, string>
>;
...
const useCRUDDestination = () => {
      ...
      .catch((error) => {
        if (error instanceof ValidationError) {
          ...
          error.inner.forEach((err) => {
            errors[err.path as keyof TDestinationErrorsField] = err.message;
          });
          setErrors(errors);
        }
      });
  ...
  const onUpdate = (id: number, payload: IDestinationPayload, cb?: () => void) => {
    destinationSchema
      .validate(payload, { abortEarly: false })
      .then((schema) => {
        makeUpdateDestination(
          ...
          {
            onSuccess: (data, variables) => {
              message.success(`Cập nhật ${variables.codeName} thành công`);
              onResetFieldsErrors();
              queryClient.invalidateQueries({
                queryKey: [queryMisc.GET_DESTINATION_LIST],
              });
      ...
      .catch((error) => {
        if (error instanceof ValidationError) {
          let errors: TDestinationErrorsField = {};
          error.inner.forEach((err) => {
            if (!errors[err.path as keyof TDestinationErrorsField]) {
              errors[err.path as keyof TDestinationErrorsField] = err.message;
            }
          });
          setErrors(errors);
    ...
    makeUpdateDestination(
      ...
      },
      {
        onSuccess: (data, variables) => {
          message.success(
            `${
              (status === Status.OK && "Kích hoạt") || (status === Status.OX && "Ngừng kích hoạt") || "Xoá"
            } thành công`,
          );
          onResetFieldsErrors();
          queryClient.invalidateQueries({
