...
import { Drawer, Space, Button, Form, Input, Checkbox, Tooltip, Switch, SwitchProps } from "antd";
...
const DrawlerDestination: React.FC<DrawlerRoleProps> = ({
  ...
  const onChangeFormData = useCallback(
    (key: keyof Omit<IDestinationPayload, "listStateProvince">, value: string) => {
      setFormData((prev) => ({
        ...prev,
        [key]: key === "codeKey" ? vietnameseTonesToUnderscoreKeyname(value).toUpperCase() : value,
      }));
    },
    [formData],
  );

  const onChangeProvinceState = useCallback((provinList: IStateProvince[]) => {
    setFormData((oldDate) => ({
      ...oldDate,
  ...
  const isButtonDisabled = useMemo(() => {
    if (isEqual(initialValues, formData) || isEmpty(formData.listStateProvince) || isEmpty(formData.codeKey)) {
      return true;
    }
    return false;
    ...
    >
      <Form layout="vertical">
        ...
        >
          <Input
            ...
            onChange={(e) => onChangeFormData("codeName", e.target.value)}
          />
        </FormItem>
        <FormItem label="Mã nhóm" validateStatus={errors?.codeKey ? "error" : ""} required help={errors?.codeKey || ""}>
          <Input
            placeholder="Mã nhóm"
            value={formData.codeKey}
            onChange={(e) => onChangeFormData("codeKey", e.target.value)}
        ...
        <FormItem label="Trạng thái" required>
          <div className="flex items-center">
            <Switch checked={formData.status === "OK"} onChange={onUpdateDestinationStatus} />
        ...
        <div className="province-wrapper">
          <ProvinceStateSelection
            ...
            editAble={actionType === EActionType.CREATE ? true : false}
          />
        </div>
      </Form>
      <div className="drawler-action absolute px-4 py-4 border-t left-0 right-0 bg-white bottom-0">
        <Space>
          ...
          <Button onClick={() => onSubmit?.(actionType, formData)} type="primary" disabled={isButtonDisabled}>
