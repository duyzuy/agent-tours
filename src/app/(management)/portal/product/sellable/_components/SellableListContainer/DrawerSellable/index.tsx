...
import { Form, Input, Select, Space, Button, SelectProps, Drawer, Tag, message, Row, Col, Divider } from "antd";
...
import InventoryExtraList, { InventoryExtraListProps } from "./InventoryExtraList";
import StockExtraSelection, { StockExtraSelectionProps } from "./StockExtraSelection";
...
export interface DrawerSellableProps {
  ...
  onSubmit?: (actionType: EActionType, formData: SellableConfirmFormData) => void;
...
const DrawerSellable: React.FC<DrawerSellableProps> = ({
  ...
  const [errorSellableForm, setErrorSellableForm] = useState<Partial<Record<keyof SellableConfirmFormData, string>>>();
    ...
    return (
      inventories?.reduce<IInventoryOption[]>((acc, inv) => {
        if (!inv.isStock && inv.productType === EProductType.TOUR) {
          acc = [...acc, { label: inv.name, value: inv.recId, data: inv }];
        }
        return acc;
      }, []) || []
    );
  ...
  const inventoriesWithManageStockAndProductTypeTour = useMemo(() => {
    return inventories?.filter((item) => item.isStock && item.productType === EProductType.TOUR) || [];
  }, [inventories]);
  ...
  const inventoriesWithManageStockAndProductTypeExtra = useMemo(() => {
    return inventories?.filter((item) => item.isStock && item.productType === EProductType.EXTRA) || [];
  }, [inventories]);
  ...
  const onSelectInventoryHasnotStock: SelectProps<number, IInventoryOption>["onChange"] = (value, options) => {
    ...
    if (isArray(options)) {
      options.forEach((opt) => {
        if (opt && opt.data) {
          inventories = [...(inventories || []), { qty: qty, inventory: opt.data }];
        }
      });
    }
    setSellableConfirmFormData((prev) => ({
  ...
  const selectExtraInventoryWithNoStock: SelectProps<number, IInventory>["onChange"] = (value, option) => {
  ...
  const inventoryExtraOptions = useMemo(() => {
    const recordIdList = (sellableConfirmFormData.extraInventories || []).map((inv) => inv.inventory.recId);
    return inventories.filter((inv) => !recordIdList.includes(inv.recId) && inv.productType === EProductType.EXTRA);
  ...
  const onAddExtraInventory = () => {
    ...
    setSellableConfirmFormData((prev) => ({
      ...prev,
      extraInventories: [...(prev.extraInventories || []), { qty: 1, inventory: inventoryExtraItem }],
    }));
    setInventoryExtraItem(undefined);
  };

  const onSelectStock: StockSelectionProps["onSetStock"] = (action, stock) => {
    /**
     * default qty = cap
    ...
    if (action === "add") {
      ...
      const { qty, stock: stockSelectedInExtraStock } =
        (extraStocks || []).find((item) => item.stock.recId === stock.recId) || {};

      if (!isUndefined(stockSelectedInExtraStock) && stockSelectedInExtraStock.open && qty) {
    ...
    if (action === "remove") {
      const indx = stockList.findIndex((item) => item.stock.recId === stock.recId);
      if (indx !== -1) {
  ...
  const onSelectStockExtra: StockExtraSelectionProps["onSetStock"] = (action, stock) => {
    /**
     * default qty = cap
     * (compare quantity open (open key) of stock); minimun >= cap
     */
    ...
    if (action === "add") {
       ...
       */
      const pickedStock = (stocks || []).find((item) => item.stock.recId === stock.recId);
  ...
  const onSaveStockExtraQuantity: StockExtraSelectionProps["onSaveQuantity"] = (record) => {
    ...
    const stockSelected = (stocks || []).find((stk) => stk.stock.recId === stock.recId);
    ...
    const stockIndx = (extraStocks || []).findIndex((stk) => stk.stock.recId === stock.recId);
    if (stockIndx !== -1) {
  ...
  const onSaveQuantityInventoryExtra: InventoryExtraListProps["onSave"] = (record) => {
    /**
    ...
    const indxInventoryExtra = sellableConfirmFormData.extraInventories.findIndex(
  ...
  const onDeleteRowInventoryExtra: InventoryExtraListProps["onDelete"] = (record) => {
    let inventoriesExtra = [...sellableConfirmFormData.extraInventories];
    const indxInventoryExtra = inventoriesExtra.findIndex((inv) => inv.inventory.recId === record.inventory.recId);
      ...
      >
        <Form layout="vertical" colon={false} labelWrap className="max-w-4xl">
          ...
          <FormItem label="Chọn nhóm kho sản phẩm" required>
            <Select
              mode="multiple"
              options={inventoryWithOutStockAndProductTypeTourOptions}
          ...
          <FormItem label="Danh sách kho sản phẩm" required>
            <div>
              <p className="text-xs text-gray-500">Số lượng ở mỗi stock mặc định theo số lượng (cap) của Sellable</p>
            </div>
            <StockSelection
              inventories={inventoriesWithManageStockAndProductTypeTour}
              scroll={{ x: 750 }}
              rowKey={"recId"}
          ...
          <FormItem
            ...
            validateStatus={errorSellableForm?.extraInventories ? "error" : ""}
          ...
          <FormItem label="Thêm kho sản phẩm (Extra)">
            <StockExtraSelection
              inventories={inventoriesWithManageStockAndProductTypeExtra}
              scroll={{ x: 750 }}
              rowKey={"recId"}
              validFrom={sellableConfirmFormData.valid}
              validTo={sellableConfirmFormData.validTo}
        ...
        <div className="bottom py-4 absolute bottom-0 left-0 right-0 border-t px-6 bg-white z-50">
          <Space>
            ...
            {actionType === EActionType.APPROVAL && initialValues?.status === Status.QQ ? (
