...
import { Drawer, Form, Input, InputNumber, Popconfirm, Divider, Row, Col, Space, Button } from "antd";
...
type EditableTableProps = TableProps<SellablePriceConfigRs["result"][0] & { index: number }>;
type PriceConfigEditingKeysType = Pick<
  SellablePriceConfigRs["result"][0],
  "adult" | "child" | "infant" | "maxAvaiable" | "avaiable" | "limitPerBooking"
>;

export type PriceConfigColumnTypes = Exclude<EditableTableProps["columns"], undefined>;
...
const DrawerPriceConfig: React.FC<DrawerTemplateSellableProps> = ({
  ...
  const { handlerSubmit, errors } = useFormSubmit<SellablePriceConfigFormData>({
  ...
  const [data, setData] = useState<(SellablePriceConfigRs["result"][0] & { index: number })[]>();

  const isEditing = (record: SellablePriceConfigRs["result"][0] & { index: number }) => record.index === editingRow;
  ...
  const isDisableSubmit = useMemo(() => {
    const dataObject = data?.reduce<{ [key: string]: PriceConfig & { index: number } }[]>((acc, item) => {
      return [
        ...acc,
        {
    ...
    const initDataObject = initPriceConfigValues?.reduce<{ [key: string]: PriceConfig }[]>((acc, item) => {
  ...
  const handleSubmitFormData = () => {
    sellableRecId && handlerSubmit({ sellableRecId, configs: data } as SellablePriceConfigFormData, onSubmit);
  };
  ...
  useEffect(() => {
    ...
    const valueWithIndex = initialValues.reduce<(SellablePriceConfigRs["result"][0] & { index: number })[]>(
      (acc, item, _index) => {
    ...
    <Drawer
      ...
      extra={
        <Space>
          ...
          <Button type="primary" onClick={handleSubmitFormData} disabled={isDisableSubmit}>
            Lưu
          </Button>
        </Space>
      }
                ...
                contents={[
                  {
                    ...
                    value: formatDate(sellableRecord.validFrom),
                  },
                  {
                    label: "Ngày kết thúc mở bán",
                    value: formatDate(sellableRecord.validTo),
                  },
                  {
                    label: "Close date",
                    value: formatDate(sellableRecord.closeDate),
                  },
                  {
                    label: "Ngày bắt đầu hiệu lực",
                    value: formatDate(sellableRecord.startDate),
                  },
                  {
                    label: "Ngày kết thúc hiệu lực",
                    value: formatDate(sellableRecord.endDate),
                  },
                ]}
              />
            </Col>
