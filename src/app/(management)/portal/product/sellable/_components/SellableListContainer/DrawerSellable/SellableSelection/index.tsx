...
import { ISellable, SellableConfirmFormData, SellableQueryParams } from "@/models/management/core/sellable.interface";
import { ITemplateSellable, TemplateSellableQueryParams } from "@/models/management/core/templateSellable.interface";
...
export type SellableSelectionProps = TableProps<ISellable> & {
  ...
  onSetSellable?: (action: "add" | "remove", sellable: SellableItemType["sellable"]) => void;
...
function SellableSelection(props: SellableSelectionProps) {
  const { sellableList, onSetSellable, onSaveQuantity, columns, ...restProps } = props;
  ...
  const { list: sellables, pageCurrent, pageSize, totalItems } = sellableResponse || {};
  const templateOptions = useMemo(() => {
    return (templateList || []).reduce<ITemplateOption[]>((acc, template) => {
      return [
        ...acc,
        {
          label: template.name,
  ...
  const onChangeTemplate: SelectProps<number, ITemplateOption>["onChange"] = (value, option) => {
  ...
  const isSelecting = (record: ISellable) => {
    return sellableList?.some((item) => item.sellable.recId === record.recId);
  };
  ...
  const getSellableSelected = (sellable: ISellable) => {
    return sellableList?.find((item) => item.sellable.recId === sellable.recId);
  };
          ...
          <>
            ...
            {isSelecting(record) ? (
              <Button
                ...
                danger
                type="text"
                onClick={() => onChangeSellable("remove", record)}
                shape="circle"
              ></Button>
            ) : record.open !== 0 ? (
              <Button
      ...
      <div className="list-select py-2">
        {isUndefined(sellableList) || isEmpty(sellableList) ? (
          ...
          <SellableList
            ...
            onDelete={(item) => onChangeSellable("remove", item.sellable)}
          />
        )}
      </div>
      <Divider />
