...
import { ISellable, SellableConfirmFormData, SellableListRs } from "@/models/management/core/sellable.interface";
import { sellableColumns } from "./sellableColumns";
import DrawerSellable, { DrawerSellableProps, EActionType } from "./DrawerSellable";
...
const SellableListContainer: React.FC<SellableListProps> = ({
  ...
  const { data: sellablePriceConfigs, isLoading: isLoadingPriceConfig } = useGetSellablePriceConfigsCoreQuery(
    drawerPriceConfig.record?.recId,
    {
      enabled: !isUndefined(drawerPriceConfig.record?.recId),
    },
  );

  const { data: sellableDetail, isLoading: isLoadingSellableDetail } = useGetSellableDetailCoreQuery(
    viewSellableDetail.recId,
    {
  ...
  const { data: inventoryListByTemplateSellableResponse } = useGetInventoryListCoreQuery({
    enabled: showDrawerApproval,
    queryParams: inventoryQueryParams,
  });
  const { list: inventoryListByTemplateSellable } = inventoryListByTemplateSellableResponse || {};
  const onCloseDrawerAndResetRecord = () => {
    setShowDrawerApproval(false);
    setEditRecord(undefined);
  };

  const handleDrawerApproval = ({ action, record }: { action: EActionType; record: SellableListRs["result"][0] }) => {
    setEditRecord(record);
    setShowDrawerApproval(true);
    setActionType(action);
  };

  const handleApproval: DrawerSellableProps["onSubmit"] = (actionType, formData) => {
    onApproval(formData, () => {
      setShowDrawerApproval(false);
      setEditRecord(undefined);
    });
  ...
  return (
    <React.Fragment>
      ...
      <TableListPage<SellableListRs["result"][0]>
        ...
        onEdit={(record) => handleDrawerApproval({ action: EActionType.EDIT, record })}
      />
      <DrawerSellable
      ...
      <DrawerSellableDetail
        ...
        data={sellableDetail}
        label={(sellableDetail && `ID${sellableDetail.sellable.recId} - ${sellableDetail?.sellable.code}`) || ""}
