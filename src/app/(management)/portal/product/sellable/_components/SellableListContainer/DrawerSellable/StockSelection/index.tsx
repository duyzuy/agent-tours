...
export type StockSelectionProps = TableProps<IStock> & {
  ...
  isLoading?: boolean;
  onSetStock?: (action: "add" | "remove", record: SellableConfirmFormData["stocks"][0]["stock"]) => void;
...
function StockSelection({
  ...
  const onChangeInventory: SelectProps<number, IInventory>["onChange"] = (value, option) => {
    setStockQueryParams((prev) => ({
      ...prev,
      requestObject: { ...prev.requestObject, inventoryId: value },
    }));
  };
  const onChangeStocks = (action: "add" | "remove", record: Partial<IStock>) => {
  ...
  const isSelecting = (record: IStock) => {
    return Boolean((stocks || [])?.find((item) => item.stock.recId === record.recId));
  };
  const getStockSelectedFromExtra = (record: IStock) => {
    return stockSelectedList?.find((item) => item.stock.recId === record.recId);
  };
  const mergeColumns: ColumnsType<IStock> = [
    ...miniStockColumns,
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Tag color={quantity < defaultQuantity ? "red" : "green"} bordered={false}>
            {isSelecting(record) ? quantity - defaultQuantity : quantity}
          </Tag>
        );
      },
    },
  ...
  return (
    <React.Fragment>
      <div className="list-select py-2">
        {isUndefined(stocks) || isEmpty(stocks) ? (
          ...
          <Space wrap className="mb-2">
            {stocks?.map((item) => (
              <Tag key={item.stock.recId} color="blue" onClose={() => onChangeStocks("remove", item.stock)} closable>
      ...
      <CustomTable
        ...
        size="small"
        dataSource={stockQueryparams?.requestObject?.inventoryId ? stockList : []}
        loading={isLoadingStock}
        pagination={{
          hideOnSinglePage: true,
          current: stockQueryparams?.requestObject?.inventoryId ? pageCurrent : 1,
          pageSize: stockQueryparams?.requestObject?.inventoryId ? pageSize : 20,
          total: stockQueryparams?.requestObject?.inventoryId ? totalItems : 0,
          size: "small",
          onChange: (page) =>
            setStockQueryParams((prev) => ({
              ...prev,
