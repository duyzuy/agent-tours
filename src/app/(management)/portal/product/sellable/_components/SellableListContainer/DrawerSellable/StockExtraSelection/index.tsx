...
export type StockExtraSelectionProps = TableProps<IStock> & {
  ...
  isLoading?: boolean;
  onSetStock?: (action: "add" | "remove", record: StockExtraItemType["stock"]) => void;
...
function StockExtraSelection(props: StockExtraSelectionProps) {
  ...
  const [stockQueryParams, setStockQueryParams] = useState(() => new StockQueryParams({ status: Status.OK }, 1, 5));
  ...
  const getStockExtraSelecting = (record: IStock) => {
    return (stocks || [])?.find((item) => item.stock.recId === record.recId);
  };
      ...
      <div className="list-select py-2">
        {isUndefined(stocks) || isEmpty(stocks) ? (
          ...
          <StockExtraList
            ...
            onDelete={(item) => onChangeStocks("remove", item.stock)}
          />
        )}
      </div>
      <Divider />
