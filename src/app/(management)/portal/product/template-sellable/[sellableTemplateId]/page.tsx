...
const Sellablepage = ({ params }: { params: { sellableTemplateId: number } }) => {
  ...
  const [sellableQueryParams, setSellableQueryParams] = useState(
    new SellableQueryParams({ sellableTemplateId: templateDetail?.recId }, 1, 20),
  );
  ...
  const tabItems: TabsProps["items"] = [
      ...
      children:
        (templateDetail && (
          <TemplateDetailcontent
            ...
            destinations={JSON.parse(templateDetail.destListJson) as IDestination[]}
            inventoryTypeList={templateDetail.inventoryTypeList.split("||") as string[]}
    ...
    },
    {
      key: "sellableForm",
      label: "Thêm sản phẩm",
      children: <SellableFormContainer template={templateDetail} onSubmit={onCreate} onCancel={onCancelCreate} />,
      icon: <PlusOutlined />,
    },
  ];
  ...
  useEffect(() => {
    if ((!templateDetail && !isLoading) || (templateDetail && templateDetail.status === Status.QQ)) {
  ...
  if (!templateDetail || (templateDetail && templateDetail.status === Status.QQ)) {
    return null;
  }
  return (
    <PageContainer
      ...
      breadCrumItems={[{ title: "Nhóm sản phẩm", href: LINKS.TemplateSellable }, { title: templateDetail.name }]}
      hideAddButton
    >
      <Tabs items={tabItems} />
    </PageContainer>
          ...
          <div className="flex-1">
            {destinations.map((des) => (
              <div key={des.id} className="mb-2">
                <div className="mb-2">
                  <p className="font-semibold">{des.codeName}</p>
                </div>
                <div className="list-state">
                  <Space wrap>
                    {des.listStateProvince.map((item, _index) => (
                      <Tag key={_index}>
                        {item.stateProvinceKey || item.countryName || item.subRegionKey || item.regionKey}
                      </Tag>
                    ))}
                  </Space>
                </div>
