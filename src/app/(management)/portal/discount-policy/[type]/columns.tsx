...
export const createDynamicDiscountColumns = (type: DiscountType) => {
  let initColumns: ColumnsType<IDiscountPolicy> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <>
            <span className="text-primary-default  font-bold">{record.code}</span>
            <p className="text-xs text-gray-500">{record.descriptions}</p>
          </>
        );
      },
    },
  ...
  initColumns = [
      ...
      render: (sysFstUpdate, record) => {
        return (
          <span className="flex items-center">
            <span>{formatDate(record.validFrom, "DD/MM/YYYY")}</span>
            <span className="mx-2">
              <SwapRightOutlined />
            </span>
    ...
    },
    {
      ...
      key: "status",
      width: 120,
      render(value, { sysFstUpdate }, index) {
        return <span>{formatDate(sysFstUpdate)}</span>;
      },
    },
  ];
  return initColumns;
