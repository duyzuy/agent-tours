...
const DrawerDiscountPolicyForm: React.FC<DrawerDiscountPolicyFormProps> = ({
  ...
  const [formData, setFormData] = useState<DiscountPolicyFormData>(initFormData);
  ...
  const onChangeValidFromAndValidToDate: RangePickerProps["onChange"] = (dates) => {
    const [dateFrom, dateTo] = dates || [];

    setFormData((oldData) => ({
      ...oldData,
      validFrom: dateFrom ? dateFrom.locale("en").format(DATE_FORMAT) : undefined,
      validTo: dateTo ? dateTo.locale("en").format(DATE_FORMAT) : undefined,
      blackoutJson: {
  ...
  const onSelectDestinations: SelectProps<string | string[], IDestination>["onChange"] = (value, destinations) => {
    setFormData((oldData) => ({
      ...oldData,
      destJson: Array.isArray(destinations) ? [...destinations] : [destinations],
    }));
  };
  ...
  const selectDayOfWeek = (dayStr: string) => {
    setFormData((oldData) => {
      ...
      const dayIndex = newDaysOfWeek?.findIndex((item) => item === dayStr);
  ...
  return (
    <Drawer
      title={discountType === DiscountType.COUPON ? "Coupon" : "Chính sách giá giảm"}
      width={550}
      onClose={onClose}
      open={isOpen}
      styles={{
    ...
    >
      <Form layout="vertical" labelWrap>
        <FormItem label="Tên" required validateStatus={errors?.name ? "error" : ""} help={errors?.name || ""}>
          <Input placeholder="Tên" value={formData.name} onChange={(ev) => onChangeFormData("name", ev.target.value)} />
        </FormItem>
        <FormItem label="Mô tả" validateStatus={errors?.descriptions ? "error" : ""} help={errors?.descriptions || ""}>
          <Input.TextArea
            placeholder="Mô tả"
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <FormItem
              ...
              required
              validateStatus={errors?.discountAmount ? "error" : ""}
              help={errors?.discountAmount || ""}
            >
              <Input
                placeholder="Số tiền giảm"
                value={formData.discountAmount}
                onChange={(ev) => onChangeFormData("discountAmount", ev.target.value)}
                maxLength={8}
              />
            </FormItem>
          </Col>
        ...
        {discountType === DiscountType.COUPON ? (
          ...
          >
            <InputNumber
              ...
              onChange={(numb) => numb && Number(numb) && numb > 0 && onChangeFormData("maxUseTimes", numb)}
              style={{ width: "100%" }}
            />
          </FormItem>
        ) : null}
        ...
        >
          <CustomRangePicker
            ...
            disabledDate={(date) => getDisabledDatesInValidToAndValidFrom(date)}
        ...
        <FormItem label="Loại trừ các ngày">
          <Row gutter={[24, 24]} className="mb-6">
            {formData.blackoutJson?.byDate?.map((dateStr, _index) => (
                ...
                <div className="flex align-baseline items-baseline">
                  <div className="mr-2 flex-1">
                    <CustomDatePicker
                      ...
                      style={{ width: "100%" }}
                      onChange={(date) => onChangeBlackDate(_index, date)}
                    />
                  </div>
                  <span className="text-red-500">
                    <MinusCircleOutlined onClick={() => onRemoveOneBlackDate(_index)} />
                  </span>
                </div>
              </Col>
            ))}
          </Row>
          <Button type="dashed" block icon={<PlusOutlined />} onClick={onAddBlackOutDate}>
            Thêm
          </Button>
        </FormItem>
        <Divider />
              ...
              <Space wrap>
                {TIME_SLOTS.map((timeSlot) => (
                  <Checkbox
                    ...
                    onChange={() => onSelectTimeSlots(timeSlot)}
                    key={timeSlot}
                    className="w-20"
                  >
                    {`${timeSlot}h`}
        ...
        <Divider />
        <FormItem>
          <Checkbox checked={formData.isValidByDayofweek} onChange={changeValidByDayOfWeek}>
            Chỉ áp dụng theo ngày trong tuần
          </Checkbox>
        </FormItem>
        {formData.isValidByDayofweek ? (
          <FormItem validateStatus={errors?.dayOfWeek ? "error" : ""} help={errors?.dayOfWeek || ""}>
        ...
        <Divider />
        <FormItem>
          <Checkbox checked={formData.isValidbyDest} onChange={onChangeApplyDestination}>
        ...
        {formData.isValidbyDest ? (
          ...
          >
            <Select<string[], IDestination>
              value={formData.destJson?.map((item) => item.codeKey)}
              fieldNames={{
                label: "codeName",
                value: "codeKey",
              }}
        ...
        <Divider />
        <FormItem>
          <Checkbox checked={formData.isValidbyTourCode} onChange={onChangeApplyTourCode}>
            Chỉ áp dụng cho code tour cụ thể
          </Checkbox>
        </FormItem>
        {formData.isValidbyTourCode ? (
          <>
            {errors?.tourCodeJson ? <p className="text-red-600">{errors.tourCodeJson}</p> : null}
            <InputTourCode value={formData.tourCodeJson || []} onChange={onChangeTourCode} />
          </>
        ) : null}
        <Divider />
      </Form>
      <Space>
        ...
        <Button
          onClick={() => handlerSubmit({ ...formData, status: Status.QQ }, onSubmit)}
          type="primary"
          ghost
          className="w-36"
        >
        ...
        <Button
          onClick={() => handlerSubmit({ ...formData, status: Status.OK }, onSubmit)}
          type="primary"
          className="w-32"
        >
          Lưu và duyệt
