...
const useCRUDCMSTemplateContentMetadata = () => {
  const { mutate: makeCreateContentAndNote } = useCreateCMSTemplateMetaContentIncludeAndNodeMutation();

  const { mutate: makeUpdateContentAndNode } = useUpdateCMSTemplateMetaContentIncludeAndNodeMutation();

  const { mutate: makeCreateItinerary } = useCreateCMSTemplateMetaContentItineraryMutation();

  const { mutate: makeUpdateItinerary } = useUpdateCMSTemplateMetaContentItineraryMutation();
  const router = useRouter();
  const queryClient = useQueryClient();
  const message = useMessage();

  const onCreateMetaContentAndIncludeAndNote = (formData: CMSTemplateContentMetaDataForm, cb?: () => void) => {
  ...
  const onUpdateMetaContentAndIncludeAndNote = (formData: CMSTemplateContentMetaDataForm, cb?: () => void) => {
  ...
  const onCreateMetaContentItinerary = (formData: CMSTemplateContentMetaDataForm, cb?: () => void) => {
  ...
  const onUpdateMetaContentItinerary = (formData: CMSTemplateContentMetaDataForm, cb?: () => void) => {
