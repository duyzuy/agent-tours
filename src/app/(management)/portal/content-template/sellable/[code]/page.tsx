...
const PageContentDetail: React.FC<PageContentDetailProps> = ({ params }) => {
  ...
  const { onUpdateTemplateContent, onUpdateStatus, onDeleteTemplateContent } = useCRUDCMSTemplateContent();
  ...
  useEffect(() => {
    if ((isUndefined(data) && !isLoading) || (!data && !isLoading)) {
      router.push("/portal/content-template/sellable/list");
    }
  }, [isLoading, data]);
  ...
  return (
    <PageContainer
      name="Template nội dung"
      hideAddButton
      onBack={() => router.push("/portal/content-template/sellable/list")}
      breadCrumItems={[
        {
          title: "Danh sách mẫu nội dung sản phẩm",
          href: "/portal/content-template/sellable/list",
        },
        { title: params.code },
      ]}
    >
