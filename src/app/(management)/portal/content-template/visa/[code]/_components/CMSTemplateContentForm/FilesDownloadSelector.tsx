...
export interface FileDownloadSelectorProps {
  ...
  setFiles?: (files: VisaTemplateContentFormData["downloads"]) => void;
}
const FileDownloadSelector: React.FC<FileDownloadSelectorProps> = ({ files = [], setFiles }) => {
            ...
            <div key={_index} className="block mb-6">
              <div className="flex items-center gap-x-4">
                <div className="flex items-center">
                  <Button type="text" icon={<DeleteOutlined />} danger onClick={() => removeFileItem(_index)}></Button>
                  <Input
                    placeholder="Tên file"
                    value={title}
                    onChange={(ev) => onChangeNameFile(_index, ev.target.value)}
                  />
                </div>
                <Button type="dashed" icon={<UploadOutlined />} onClick={() => showMediaUpload(_index)}>
                  Chọn file
                </Button>
              </div>
              {isEmpty(link) ? null : (
                <p className="mt-2 line-clamp-1 max-w-sm">
                  <Link href={`${mediaConfig.rootApiPath}/${link}`} className="flex items-center" target="_blank">
                    <LinkOutlined />
                    <span className="line-clamp-1 ml-2 text-xs">{link}</span>
                  </Link>
                </p>
              )}
            </div>
