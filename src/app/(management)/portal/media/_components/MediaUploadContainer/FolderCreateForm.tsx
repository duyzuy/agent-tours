...
import { IMediaFolderListRs, IMediaFolderPayload } from "@/models/management/media.interface";
...
const FolderCreateForm: React.FC<FolderCreateFormProps> = ({
  ...
  const onChangeFolderName = (key: keyof IMediaFolderPayload, value: string) => {
    setFormdata((prev) => ({
      ...prev,
      [key]: value,
    ...
    return [
      {
        ...
        parentSlug: "",
        depth: 0,
        children: formatFolderListToOptionDataSelectTree(folderList, mediaConfig.rootFolder, 1),
      },
    ];
  }, [folderList]);
  ...
  return (
    <Form layout="vertical" disabled={isLoading}>
      ...
      <Space>
        ...
        <Button onClick={() => handlerSubmit(formData, onSubmitForm)} type="primary" loading={isLoading}>
          Thêm mới
        </Button>
      </Space>
    </Form>
...
const formatFolderListToOptionDataSelectTree = (
  ...
  return items.map((item) => {
    ...
    if (!isEmpty(item.children)) {
      childs = formatFolderListToOptionDataSelectTree(item.children, item.folderSlug, depth + 1);
    }
