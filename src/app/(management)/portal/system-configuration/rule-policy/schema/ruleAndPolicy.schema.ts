...
export const depositRuleAndPolicyCreateSchema: ObjectSchema<DepositRuleAndPolicyFormData> = object({
  type: string().oneOf<PolicyType>([PolicyType.DEPOSIT]).required("Chưa chọn loại."), //lấy từ core
  typeName: string(),
  cat: string()
    .oneOf<PolicyCat>([PolicyCat.BYAGENT, PolicyCat.BYDESTINATION, PolicyCat.BYTOURCODE], "Cat không hợp lệ.")
...
export const limitBookingTimeRuleAndPolicyCreateSchema: ObjectSchema<LimitTimeBookingRuleAndPolicyFormData> = object({
  type: string().oneOf<PolicyType>([PolicyType.BOOKING_TIMELIMIT]).required("Chưa chọn loại."), //lấy từ core
  typeName: string(),
  cat: string()
    .oneOf<PolicyCat>([PolicyCat.BYDESTINATION, PolicyCat.BYTOURCODE], "Cat không hợp lệ.")
    .required("Chưa chọn loại."), //lấy từ core
  catName: string().required("Không bỏ trống."), //lấy từ core
  rule: string().oneOf<PolicyRule>([PolicyRule.HOURSAFTER_BOOK], "Rule không hợp lệ.").required("Chưa chọn quy tắc."),
...
export const penaltyRuleAndPolicyCreateSchema: ObjectSchema<PenaltyRuleAndPolicyFormData> = object({
  type: string().oneOf<PolicyType>([PolicyType.PENALTY]).required("Chưa chọn loại."), //lấy từ core
  typeName: string(),
  cat: string()
    .oneOf<PolicyCat>([PolicyCat.BYDESTINATION, PolicyCat.BYTOURCODE], "Cat không hợp lệ.")
    .required("Chưa chọn loại."), //lấy từ core
