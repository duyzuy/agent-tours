...
const PageManagement = () => {
  ...
  const [queryParams, setQueryParams] = useState(() => new PageContentQueryParams(undefined, 1, 10));
  const { data: pageContentData, isLoading } = useGetPageContentListQuery(queryParams);
  ...
  const pageListContents = useMemo(() => getPageListWithoutChild(pageContentData?.list || []), [pageContentData]);
  ...
  return (
    ...
    >
      <TableListPage<PageContentDataType>
        ...
        columns={columns}
        isLoading={isLoading}
        onEdit={(record) => router.push(`/portal/contents/page/${record.originId}`)}
        pagination={{
