...
const InputTourCode: React.FC<InputTourCodeProps> = ({ value, onChange }) => {
  ...
  const handleEditInputChange: InputProps["onChange"] = (ev) => setEditInputValue(ev.target.value);
  ...
  return (
    <>
        ...
        <Space wrap>
          {codes.map((code, _index) => {
            if (_index === editInputIndex) {
              ...
              return (
                <Tag key={code} closable={true} style={{ userSelect: "none" }} onClose={() => handleRemoveCode(code)}>
                  <span
                    onDoubleClick={(e) => {
                  ...
                  >
                    {isLongCode ? `${code.slice(0, 20)}...` : code}
                  </span>
                </Tag>
              );
            }
      ...
      {inputVisible ? (
        ...
        <Tag onClick={showInputTourCode} style={{ borderStyle: "dashed", cursor: "pointer" }}>
