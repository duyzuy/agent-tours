...
import { DisCountQueryParams, DiscountType, IDiscountPolicy } from "@/models/management/core/discountPolicy.interface";
...
const DiscountPolicyPage = ({ params }: DiscountPolicyPageProps) => {
  ...
  const { data: discountResponse, isLoading: isLoadingList } = useGetDiscountPolicyListCoreQuery({
    queryParams: queryParams,
    enabled: true,
  });

  const onChangePagination = useCallback<Required<PaginationProps>["onChange"]>(
    (page, pageSize) => {
  ...
  useEffect(() => {
    if (type !== lowerCase(DiscountType.COUPON) && type !== lowerCase(DiscountType.POLICY)) {
      router.push(`./portal/discount-policy/${lowerCase(DiscountType.POLICY)}`);
    }
  }, []);
    ...
    <React.Fragment>
      ...
      >
        <TableListPage<IDiscountPolicy>
          ...
          hideApproval={(record) => record.status === Status.OK || record.status === Status.EX}
          hideDelete={(record) => record.status === Status.EX}
          onDelete={({ recId }) => onDeactive({ recId })}
          fixedActionsColumn={false}
        />

        <ModalDiscountDetail isOpen={isShowDetail} onClose={hideDiscountDetail} record={discountDetail} />
