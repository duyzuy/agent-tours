...
import { IRuleAndPolicyCat, IRuleAndPolicyRule } from "@/models/management/core/ruleAndPolicy.interface";
...
const DepoSitCreateForm: React.FC<DepoSitCreateFormProps> = ({ onSubmit }) => {
  const { data: catList, isLoading: isLoadingCat } = useGetRuleAndPolicyDepositCatListCoreQuery({ enabled: true });
  const { data: ruleList, isLoading: isLoadingRule } = useGetRuleAndPolicyDepositRuleListCoreQuery({ enabled: true });
  ...
  const { handlerSubmit, errors } = useFormSubmit<DepositRuleAndPolicyFormData>({
  ...
  const handleSubmitForm: HandleSubmit<DepositRuleAndPolicyFormData> = (data) => {
    onSubmit?.(data, () => {
