...
export interface LimitTimeCreateFormProps {
  onSubmit?: (data: LimitTimeBookingRuleAndPolicyFormData, cb?: () => void) => void;
}

type RequiredLimitBookingTimeFormData = Required<LimitTimeBookingRuleAndPolicyFormData>;
const LimitTimeCreateForm: React.FC<LimitTimeCreateFormProps> = ({ onSubmit }) => {
  const { data: catList, isLoading: isLoadingCat } = useGetRuleAndPolicyLimitTimeCatListCoreQuery({ enabled: true });
  const { data: ruleList, isLoading: isLoadingRule } = useGetRuleAndPolicyLimitTimeRuleListCoreQuery({ enabled: true });
  ...
  const { handlerSubmit, errors } = useFormSubmit<LimitTimeBookingRuleAndPolicyFormData>({
  ...
  const handleSubmitForm: HandleSubmit<LimitTimeBookingRuleAndPolicyFormData> = (data) => {
    onSubmit?.(data, () => {
  ...
  return (
    <div className="form max-w-2xl">
      ...
      >
        ...
        <FormItem label="Số giờ" required validateStatus={errors?.soGio ? "error" : ""} help={errors?.soGio || ""}>
          <Input
