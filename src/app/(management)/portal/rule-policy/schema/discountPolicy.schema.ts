...
export const discountPolicySchema: ObjectSchema<DiscountPolicyFormData> = object({
  ...
  type: string().oneOf<DiscountType>([DiscountType.COUPON, DiscountType.POLICY]),
  code: string().required("Mã tour không bỏ trống."),
  maxUseTimes: number().when("type", {
    is: DiscountType.COUPON,
    then: (schema) => schema.min(1, "Số lần sử dụng tối thiểu là 1.").required("Số lần sử dụng không bỏ trống."),
      ...
      schema
        .of(
          object({
            ...
            id: number().required(),
            codeKey: string().required("Code key không bỏ trống."),
            codeName: string().required("Code name không bỏ trống."),
            status: string().oneOf<Status>([Status.OK]).required(),
            listStateProvince: array()
              .of(
                object({
                  ...
                  cat: string()
                    .oneOf<
                      "REGIONLIST" | "SUBREGIONLIST" | "COUNTRYLIST" | "STATEPROVINCELIST"
                    >(["REGIONLIST", "SUBREGIONLIST", "COUNTRYLIST", "STATEPROVINCELIST"])
  ...
  tourCodeJson: array().when("isValidbyTourCode", {
    is: true,
    then: (schema) => schema.of(string().required("Không bỏ trống.")).min(1, "Nhập ít nhất 1 mã tour."),
    otherwise: (schema) => schema.optional(),
  }),
  ...
  timeJson: array().when("isValidByTime", {
    is: true,
    then: (schema) =>
      schema.of(number().required("Khung thời gian không bỏ trống.")).min(1, "Vui lòng chọn khung thời gian"),
    otherwise: (schema) => schema.optional(),
  }),
  isValidByDayofweek: boolean().default(false),
  dayOfWeek: array().when("isValidByDayofweek", {
    is: true,
    then: (schema) => schema.of(string().required("Khung thời gian không bỏ trống.")).min(1, "Vui lòng chọn ngày"),
    otherwise: (schema) => schema.optional(),
  }),
  discountAmount: number().required("Số tiền giảm không bỏ trống.").min(1, "Số tiền giảm tối thiểu lớn hơn 1"),
