...
const useCRUDDiscountPolicy = () => {
  ...
  const onActive = (payload: { recId: number } | { code: string } | undefined, cb?: () => void) => {
    doActive(payload, {
      onSuccess(data, variables, context) {
  ...
  const onDeactive = (payload: { recId: number } | { code: string } | undefined, cb?: () => void) => {
    doDeactive(payload, {
      onSuccess(data, variables, context) {
  ...
  const convertFormDataToPayload = (formData: DiscountPolicyFormData): IDiscountPolicyPayload => {
    let {
      name,
      descriptions,
      destJson,
    ...
    const blackoutByDate = blackoutJson?.byDate?.reduce<string[]>((acc, item) => {
      if (item) {
        acc = [...acc, item];
      }
      return acc;
    ...
    const blackoutByDateRange = blackoutJson?.byDaterange?.reduce<{ fromDate: string; toDate: string }[]>(
      (acc, item) => {
        if (item && item.fromDate && item.toDate) {
          acc = [...acc, { fromDate: item.fromDate, toDate: item.toDate }];
        }
        return acc;
      },
      [],
