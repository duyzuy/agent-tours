    ...
    return {
      ...
      expandedRowRender: (record) => (
        <div>
          ...
          <Space wrap>
            {(JSON.parse(record.destJson) as IStateProvince[]).map((item, _index) => {
              if (item.cat === "REGIONLIST") {
                return (
                  <Tag color="red" key={_index}>
                    {item.regionKey}
                  </Tag>
                );
              }
              if (item.cat === "SUBREGIONLIST") {
                return (
                  <Tag color="pink" key={_index}>
                    {item.subRegionKey}
                  </Tag>
                );
              }
              if (item.cat === "STATEPROVINCELIST") {
                return <Tag key={_index}>{item.stateProvinceKey}</Tag>;
              }
              return (
