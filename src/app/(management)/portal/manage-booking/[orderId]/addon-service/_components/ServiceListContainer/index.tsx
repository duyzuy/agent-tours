...
import { useDispatchManageBooking, useSelectorManageBooking } from "../../../hooks/useManageBooking";
...
const ServiceListContainer: React.FC<ServiceListContainerProps> = ({
  ...
  const onAddSSRBooking = useCallback<Required<ServiceItemProps>["onConfirm"]>((ssrItems, ssrItemRemove, serviceId) => {
    dispatch(
      addSSROrder({
  ...
  const gotoPaymentPage = () => {
    startTransition(() => router.push(`./portal/manage-booking/${orderId}/addon-service/payment`));
  };
    ...
    <>
      <Row gutter={60}>
        <Col span={15}>
          ...
          {/* {Object.entries(groupingServices)?.map(([key, { serviceId, serviceName, priceConfigs }]) => (
            <ServiceItem
              key={serviceId}
              ...
              initSSRBookingItems={editSSROrder?.bookingDetails ? editSSROrder.bookingDetails[serviceId]?.items : []}
              initSSRBookingItemsRemove={editSSROrder.bookingSsrDelete}
