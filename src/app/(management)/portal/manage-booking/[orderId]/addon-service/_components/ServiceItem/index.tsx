...
const ServiceItem: React.FC<ServiceItemProps> = ({
  ...
  //     return ssrListBooked?.filter((bookingSSrItem) => bookingSSrItem.config.sellableDetailsId === serviceId) || [];
  //   }, [ssrListBooked]);

  //   const ssrItemGroupByPassenger = useMemo(() => {
  //     return bookingSSRItemsOfService.reduce<BookingSSRItemsByPassengerType[]>((acc, bookingSSRItem) => {
  //       const { pax } = bookingSSRItem;

  //       const paxIndex = acc.findIndex((item) => item.pax.recId === pax.recId);

  //       if (paxIndex !== -1) {
  //         const newSSr = initSSRBookingItemsRemove.some((item) => item.recId === bookingSSRItem.recId)
  //           ? [...acc[paxIndex].ssr]
  ...
  //         });
  //       } else {
  //         const ssr = initSSRBookingItemsRemove.some((item) => item.recId === bookingSSRItem.recId)
  //           ? []
  //           : [bookingSSRItem];
  ...
  //       (acc, bookingSSRItem) => {
  //         if (initSSRBookingItemsRemove.some((item) => item.recId === bookingSSRItem.recId)) {
  //           acc = {
  ...
  //               accOfPax = {
  //                 subTotal: accOfPax.subTotal + priceConfig[type] * quantity,
  ...
  //         itemCount: total.itemCount + subTotalPricingItemsAddNew.itemCount,
  //       };
  //     }
  //     return total;
  //   }, [initSSRBookingItemsRemove, initSSRBookingItems, bookingSSRItemsOfService]);
        ...
        <div className="service__item-inner">
          <div className="service__item-head px-6 py-6 ">
            <div className="flex justify-between">
              ...
              <Button type="primary" ghost size="small" onClick={showDrawer} className="cursor-pointer">
                Chọn
              </Button>
            </div>
            {/* <div className="border-t pt-6 mt-6">
              <div className="flex items-center justify-between">
                ...
                >
                  {moneyFormatVND(subtotalServiceItem?.subTotal)}
                  <span className="font-[10px] ml-2">
                    {showDetail ? <RightOutlined className="w-3" /> : <RightOutlined className="w-3" />}
                  </span>
                </span>
              </div>
            </div> */}
          ...
          {showDetail ? (
            <div className="break-down-service-item">
              {/* {ssrItemGroupByPassenger ? (
                <div className="pax__item__ssr-old border-t py-4 px-6">
                  ...
                  <div>
                    ...
                    {ssrItemGroupByPassenger.map(({ pax, ssr }, _index) => (
                      <BreakDownItem
                        key={`pax-${_index}`}
                        paxFullName={
                          getFullnamePassenger(pax.paxLastname, pax.paxMiddleFirstName) ?? `Hành khách ${_index + 1}`
                        }
                        pricingItems={ssr.map((item) => ({
                          recId: item.recId,
                          ssrClass: item.class,
                          quantity: 1,
                          price: moneyFormatVND(item.amount),
                        }))}
                      />
                    ))}
                  </div>
              ...
              <div className="pax__item__ssr-new py-4 px-6 border-t">
                ...
                {initSSRBookingItems?.map(({ booking, ssr }, _index) => (
                  <BreakDownItem
                    key={`pax-${_index}`}
                    paxFullName={
                      getFullnamePassenger(booking.pax.paxLastname, booking.pax.paxMiddleFirstName) ??
                      `Hành khách ${_index + 1}`
                    }
                    pricingItems={ssr.map(({ quantity, priceConfig, type }) => ({
                      recId: priceConfig.recId,
                      ssrClass: priceConfig["class"],
                      quantity: quantity,
                      price: moneyFormatVND(priceConfig[type]),
                    }))}
                  />
                ))}
              </div>
...
const BreakDownItem = function ({ paxFullName, pricingItems }: BreakDownItemProps) {
  return (
    <div className="flex">
      <span className="passenger-fullname w-36">{paxFullName}</span>
      <div className="ssr-item-list">
        {pricingItems.length
          ? pricingItems.map(({ recId, ssrClass, quantity, price }) => (
