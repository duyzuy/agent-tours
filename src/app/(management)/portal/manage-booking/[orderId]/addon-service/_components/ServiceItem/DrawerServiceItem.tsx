...
const DrawerServiceItem: React.FC<DrawerServiceItemProps> = ({
  ...
  const [bookingItemSelecting, setBookingItemSelecting] = useState<BookingDetailItemType>();
  ...
  const handleChangeServiceItem = (action: "minus" | "plus", quantity: number, priceConfig: PriceConfig) => {
    if (quantity < 0) {
      return;
    }
    ...
    const totalSellableConfigSelecting = getTotalSellableConfigSelecting(priceConfig.recId);

    if (action === "plus" && totalSellableConfigSelecting + 1 > priceConfig.open) {
      message.info("Số lượng dịch vụ đã hết.");
      return;
    }
    const bookingItemIndex = addList.findIndex((item) => item.booking.recId === bookingItemSelecting.recId);
    ...
    if (bookingItemIndex !== -1) {
      ...
      const ssrIndex = ssrListByBookingItem.findIndex((ssrItem) => ssrItem.priceConfig.recId === priceConfig.recId);
      /**
  ...
  const getQuanitySSROfPaxByPriceConfig = (priceConfig: PriceConfig) => {
    ...
    const bookingItem = addList.find((bkItem) => bkItem.booking.recId === bookingItemSelecting?.recId);

    const ssrItem = bookingItem?.ssr.find((ssrItem) => ssrItem.priceConfig.recId === priceConfig.recId);
  ...
  const getTotalQuantitySSROfOnePaxBooking = (paxBookingId: number) => {
    ...
    const totalInitAddedBefore = bookingSSRItemsBooked.reduce((totalQty, itemRv) => {
      if (itemRv.bookingRefId === paxBookingId && removeList.every((item) => item.recId !== itemRv.recId)) {
        totalQty += 1;
      }
      return totalQty;
    }, 0);
  ...
  const subTotal = useMemo(() => {
    ...
    const totalNewAdded = addList.reduce((subTotal, bkItem) => {
      bkItem.ssr.forEach((ssrItem) => {
        subTotal += ssrItem.quantity * ssrItem.priceConfig[ssrItem.type];
      });
      return subTotal;
    }, 0);
  ...
  const bookingSSRItemsBookedByCurrentPax = useMemo(
    () => bookingSSRItemsBooked.filter((item) => item.bookingRefId === bookingItemSelecting?.recId),
    [bookingSSRItemsBooked, bookingItemSelecting],
  );

  const getFullnamePassenger = useCallback((lastName?: string, middleAndFirstName?: string) => {
    if (middleAndFirstName && !isEmpty(middleAndFirstName) && lastName && !isEmpty(lastName)) {
  ...
  const handleConfirmService = () => {
    serviceId && onConfirm?.(ssrData.addList, ssrData.removeList, serviceId);
    onClose?.();
    // bookingItems && setBookingItemSelecting(bookingItems[0]);
  };
  ...
  const hasBookingSSRItemRemoved = useCallback(
    (bkDetailSSRItem: BookingDetailSSRItemType) => {
      return ssrData.removeList.some((item) => item.recId === bkDetailSSRItem.recId);
    },
    [ssrData],
  );
  const onAddRemoveSSRItem: BookingSSRItemprops["onAdd"] = (data) => {
    ...
    setSSRData((oldData) => {
      ...
      const indexItem = removeList.findIndex((item) => item.recId === data?.recId);
      ...
      <div className="service__item-top py-4 px-6 bg-slate-100 overflow-auto">
        <div className="booking-list flex gap-x-4">
          {bookingItems?.map((bkItem, _index) => (
            <div
              className={classNames("pax__item p-3 border bg-white shadow-sm rounded-md border-b-2", {
                "border-primary-default": bkItem.recId === bookingItemSelecting?.recId,
                "cursor-pointer": bkItem.recId !== bookingItemSelecting?.recId,
              })}
              key={_index}
              onClick={() => selectBookingItem(bkItem)}
            >
              <div className="w-32">
                <span className="text-xs">{`${getPassengerType(bkItem.type)}`}</span>
                <span className="block font-[500]">
                  {getFullnamePassenger(bkItem.pax.paxLastname, bkItem.pax.paxMiddleFirstName) ??
                    `Hành khách ${_index + 1}`}
                </span>

                {getTotalQuantitySSROfOnePaxBooking(bkItem.recId) === 0 ? (
                  "--"
                ) : (
                  <span className="w-5 h-5 bg-primary-default text-white inline-flex items-center justify-center rounded-full text-xs">
                    {getTotalQuantitySSROfOnePaxBooking(bkItem.recId)}
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="service-item-booked mb-6">
          {bookingSSRItemsBookedByCurrentPax ? (
            <div>
              ...
              {bookingSSRItemsBookedByCurrentPax.map((bookingSSRItem, _index) => (
                <BookingSSRItem
                  key={_index}
                  isRemoved={hasBookingSSRItemRemoved(bookingSSRItem)}
                  data={bookingSSRItem}
        ...
        <div className="service-items-new">
          ...
          {pricingConfigs.map((item, _index) => (
            <SellableConfigItem
              ...
              open={item.open}
              price={bookingItemSelecting ? moneyFormatVND(item[bookingItemSelecting?.type]) : undefined}
              onChange={(action, value) => handleChangeServiceItem(action, value, item)}
        ...
        <div className="flex justify-between">
          <div>
            <div>
              ...
              <span className="font-[500] text-[16px] text-primary-default">{moneyFormatVND(subTotal)}</span>
            </div>
          </div>
          <Space>
            <Button
