...
const ManageBookingOrderSummary: React.FC<ManageBookingOrderSummaryProps> = ({
  ...
  const orderSSREdit = useSelectorManageBooking((state) => state.editSSROrder);
      ...
      <div className="manage__booking-order-summary-body mb-6">
          ...
          <div className="box__passenger-body px-6 pb-6">
            {bookingDetails?.map(({ pax, recId, amount }) => (
              <div className="box-item" key={recId}>
                <div className="flex">
                  <span className="w-32">{`${pax?.paxLastname}, ${pax.paxMiddleFirstName}`}</span>
                  <span className="w-8">1 x</span>
                  <span className="text-primary-default flex-1 text-right">{moneyFormatVND(amount)}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {/* {groupingServices
              ? Object.entries(groupingServices)?.map(([key, { serviceId, serviceName, items }]) => (
                  <div key={serviceId} className="mb-3">
                    ...
                    <div>
                      {items.map((item) => (
                        <div className="flex justify-between" key={item.recId}>
                          <span className="flex-1">{item.class}</span>
                          <span className="w-8">1 x</span>
                          <span className="text-primary-default text-right w-28">{moneyFormatVND(item.amount)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="sub-total border px-6 py-6 rounded-lg">
          <div className="flex justify-between items-center">
            <span>Tổng tiền</span>
            <span className="text-primary-default text-lg">{moneyFormatVND(bookingOrder?.totalAmount)}</span>
          </div>
        </div>
      </div>
    </>
