...
const ManageBookingReservationPage: React.FC<ManageBookingReservationPageProps> = ({ params }) => {
  const orderDetailInformation = useSelectorManageBooking((state) => state.order);
        ...
        <div className="customer__page">
          ...
          <div className="customer__information bg-white rounded-md max-w-xl mx-auto mb-6 drop-shadow-sm">
            ...
            <div className="customer__information-body px-6 pt-6 pb-3">
              ...
              <DetailItem title="Tên tour" value={bookingOrder?.template?.name} />
              <DetailItem title="Mã Tour" value={bookingOrder?.sellable?.code} />
              <DetailItem
                title="Ngày đi"
                value={bookingOrder?.sellable?.startDate ? formatDate(bookingOrder.sellable?.startDate) : undefined}
              />
              <DetailItem
                title="Ngày về"
                value={bookingOrder?.sellable?.endDate ? formatDate(bookingOrder.sellable?.endDate) : undefined}
              />
            </div>
          </div>
          <div className="customer__information bg-white rounded-md max-w-xl mx-auto mb-6 drop-shadow-sm">
          ...
          <div className="pax__information bg-white rounded-md max-w-xl mx-auto mb-6 drop-shadow-sm">
            <div className="customer__information-head border-b px-6 py-6">
              <h3 className="font-bold text-lg">Thông tin hành khách</h3>
            </div>
            <div className="px-6 pt-6 pb-3">
              {bookingDetails?.map(({ booking }, _index) => (
                <React.Fragment key={_index}>
                  <div>
                    <DetailItem
                      title={getPassengerType(booking.pax.type) || ""}
                      value={`${booking.pax.paxLastname}, ${booking.pax.paxMiddleFirstName}`}
                    />
                    <DetailItem title={"Giá tiền"} value={moneyFormatVND(booking.amount)} />
                  </div>
                </React.Fragment>
              ))}
            </div>
          ...
          <div className="pax__information bg-white rounded-md max-w-xl mx-auto mb-6 drop-shadow-sm">
            ...
            <div className="px-6 pt-6 pb-3">
              {/* {ssrGroupedByService
                ? Object.entries(ssrGroupedByService).map(([key, value]) => (
                    <div key={key} className="mb-3">
                      <div className="mb-3">
                        <p className="font-[500]">{value.serviceName}</p>
                      </div>
                      <div>
                        {value.items.map((item, _index) => (
                          <div className="flex items-center" key={`${value.serviceId}-${_index}`}>
                            <span className="block w-32">{item.class}</span>
                            <span>{`1x ${moneyFormatVND(item.amount)}`}</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="customer__information bg-white rounded-md max-w-xl mx-auto drop-shadow-sm">
            ...
            <div className="customer__information-body px-6 pt-6 pb-3">
              ...
              <DetailItem title="Phí bổ sung" value={moneyFormatVND(bookingOrder?.extraPrice)} />
              <DetailItem title="Phí" value={moneyFormatVND(bookingOrder?.charge)} />
              <DetailItem title="Đã thanh toán" value={moneyFormatVND(bookingOrder?.totalPaid)} />
              <DetailItem
                title="Tổng tiền"
                value={
          ...
          <div className="customer__information py-4 rounded-md max-w-xl mx-auto">
            <div className="flex justify-center">
              <Space>
                <Button onClick={() => router.push("./portal/booking")}>Về trang đặt chỗ</Button>
                <Button type="primary" onClick={() => router.push(`./portal/manage-booking/${bookingOrder?.recId}`)}>
                  Quản lý đặt chỗ
                </Button>
              </Space>
            </div>
