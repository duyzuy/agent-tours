...
const ManageBookingEditSSRPaymentPage: React.FC<AddOnServiceProps> = ({ params }) => {
  const router = useRouter();

  const orderDetail = useSelectorManageBooking((state) => state.order);
  const editSSROrder = useSelectorManageBooking((state) => state.editSSROrder);
  ...
  useEffect(() => {
    if (
      (!editSSROrder.bookingSsrDelete.length && !editSSROrder.bookingDetails) ||
      (!editSSROrder.bookingSsrDelete.length &&
        editSSROrder.bookingDetails &&
        Object.entries(editSSROrder.bookingDetails).every(([key, values]) =>
  ...
  return (
    <PageContainer
      ...
      onBack={() => router.push(`./portal/manage-booking/${params.orderId}/addon-service`)}
    ...
    >
      <div className="manage__booking-payment w-[550px]">
        <div className="mb-6 border-b pb-6">
          ...
          <div>
            <span className="block">{bookingOrder?.template.name}</span>
            <span className="text-xs text-gray-600">{bookingOrder?.sellable.code}</span>
          </div>
          <div className="pt-4">
            <div className="line h-[1px] bg-gray-100" style={{ width: "calc(100% - 144px)" }}></div>
              ...
              <div className="date__box-item w-36">
                ...
                <span className="mt-2 block">
                  ...
                  <span className="date-value">
                    {bookingOrder?.sellable.startDate ? formatDate(bookingOrder?.sellable.startDate) : ""}
                  </span>
                </span>
              </div>
              <div className="date__box-item w-36">
                ...
                <span className="mt-2 block">
                  ...
                  <span>{bookingOrder?.sellable.endDate ? formatDate(bookingOrder?.sellable.endDate) : ""}</span>
                </span>
              </div>
            </div>
          </div>
          ...
          <div>
            {orderDetail?.bookingDetails
              ? orderDetail?.bookingDetails.map(({ booking }) => (
                  <div className="flex justify-between" key={booking.recId}>
                    <span>{`${booking.pax.paxLastname}, ${booking.pax.paxMiddleFirstName}`}</span>
                    <span className="text-primary-default">{moneyFormatVND(booking.amount)}</span>
                  </div>
                ))
              : "--"}
          </div>
        ...
        <div className="border-b mb-6 pb-6">
          ...
          <div>
            {/* {groupingServices
              ? Object.entries(groupingServices)?.map(([key, { serviceId, serviceName, items }]) => (
                  <div key={serviceId} className="mb-3 flex">
                    ...
                    <div className="flex-1">
                      ...
                      {items.map((item, _index) => (
                        <div className="flex justify-between" key={_index}>
                          <span className="w-16">{item.class}</span>
                          <span className="w-16 text-center">1 x</span>
                          <span className="text-primary-default">{moneyFormatVND(item.amount)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <div className="manage__booking-payment-summary mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-primary-default">{moneyFormatVND(subTotal.totalAmountOldOrder)}</span>
          </div>
          <div className="flex justify-between items-center mb-2">
            <span>Đã thanh toán</span>
            <span className="text-primary-default">{moneyFormatVND(subTotal.totalPaid)}</span>
          </div>
          <div className="flex justify-between items-center mb-2">
            <span>Tổng tiền chặng mới</span>
            <span className="text-primary-default">{moneyFormatVND(subTotal.totalAmountNewOrder)}</span>
          </div>
          <div className="flex justify-between items-center mb-2">
            <span>Phí bổ sung</span>
            <span className="text-primary-default">{moneyFormatVND(subTotal.totalCharge)}</span>
          </div>
          <div className="flex justify-between items-center mb-2">
            <span>Số tiền còn phải thanh toán</span>
            <span className="text-primary-default">{moneyFormatVND(subTotal.totalAmountNeedToPay)}</span>
          </div>
        </div>
        <div className="manage__booking-service-actions bg-white mb-6">
          <Space>
            ...
            <Button type="primary" className="w-48" onClick={() => onUpdateSSRByPax()} loading={isLoading}>
              Thanh toán
            </Button>
          </Space>
        </div>
