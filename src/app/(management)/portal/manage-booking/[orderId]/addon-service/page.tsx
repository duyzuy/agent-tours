...
const AddonServicePage: React.FC<AddOnServiceProps> = ({ params }) => {
  ...
  const { data: serviceList, isLoading: isLoadingServices } = useGetBookingTourServicesCoreQuery({
    enabled: !isUndefined(orderDetailInformation?.bookingOrder.sellableId),
    sellableId: orderDetailInformation?.bookingOrder.sellableId,
  });

  const bookingDetails = useMemo(() => {
    return orderDetailInformation?.bookingDetails?.reduce<BookingDetailItemType[]>((acc, bookingItem) => {
  ...
  const ssrListBooked = useMemo(() => {
    return orderDetailInformation?.ssr?.reduce<BookingDetailSSRItemType[]>((acc, bookingItem) => {
      acc = [...acc, bookingItem.booking];
      return acc;
    }, []);
  }, [orderDetailInformation]);
      ...
      <div className="manage__booking-service w-[1040px]">
        {!isEmpty(serviceList) ? (
          ...
          <EmptyService
            ...
            onClick={() => router.push(`/portal/manage-booking/${params.orderId}`)}
          />
        )}
      </div>
    </PageContainer>
