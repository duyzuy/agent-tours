...
const ServiceDetail: React.FC<ServiceDetailProps> = ({
  ...
  //         const bookingPassengerIndex = bookingPassengerList.findIndex((pax) => pax.recId === bookingSSRItem.pax.recId);
  //         /**
  ...
  //             ssr: [...bookingPassengerList[bookingPassengerIndex].ssr, bookingSSRItem],
  //           });
  //         } else {
  ...
  return (
    ...
    >
      <div className="booking__detail-head mb-6">
        <span className="text-[16px] font-semibold mr-2">Thông tin dịch vụ</span>
        <Button icon={<PlusCircleFilled />} type="primary" size="small" onClick={onBuyService} loading={isLoading}>
          Thêm dịch vụ
        </Button>
      </div>
      <div className="booking__detail-body">
        {/* {!serviceListGroupingByPax ? (
          ...
          <Row gutter={[24, 24]}>
            {Object.entries(serviceListGroupingByPax).map(([key, { serviceName, bookingPassengerList, serviceId }]) => (
                ...
                <div className="service__item mb-3 border px-4 py-4 rounded-md">
                  ...
                  <div className="service__item-body">
                    <div className="service__item-pax-list">
                      ...
                      {bookingPassengerList.map(({ recId, passengerInfo, ssr }, _index) => (
                        <div className="service__item-pax-item flex items-start py-2" key={recId}>
                          <span className="pax-type w-24 xl:w-36 font-[500] pr-2">
                            {getPassengerType(passengerInfo.type)}
                          </span>

                          <span className="w-20 xl:w-32 font-[500] pr-2">
                            {isEmpty(passengerInfo.paxLastname) ? "--" : passengerInfo.paxLastname}
                          </span>
                          <span className="w-36 xl:w-48 font-[500] pr-2">
                            {isEmpty(passengerInfo.paxMiddleFirstName) ? "--" : passengerInfo.paxMiddleFirstName}
                          </span>
                          <div className="price-config-items">
                            {ssr.map((item, _indexPrc) => (
                              <div className="flex" key={_indexPrc}>
                                <span className="block w-16 xl:w-24 pr-2">{item.config.class}</span>
                                <span className="block w-20 pr-2">{`1 x`}</span>
                                <span className="block w-32 pr-2 text-primary-default">
                                  {moneyFormatVND(item.config[passengerInfo.type])}
                                </span>
                              </div>
                            ))}
                          </div>
...
const BookingPassengerServiceItem: React.FC<BookingPassengerServiceItemProps> = ({ passengers }) => {
  return (
    <div>
      <div className="service__item-pax-item-head flex items-center text-xs text-gray-600">
        <span className="w-24 xl:w-36 pr-2">Hành khách</span>
      ...
      {passengers.map(({ pasengerType, paxLastname, paxMiddleFirstName, priceConfigItems }, _index) => (
        <div className="service__item-pax-item flex items-center py-1" key={_index}>
          <span className="pax-type w-24 xl:w-36 font-[500] pr-2">{pasengerType}</span>

          <span className="w-20 xl:w-32 font-[500] pr-2">{isEmpty(paxLastname) ? "--" : paxLastname}</span>
          <span className="w-36 xl:w-48 font-[500] pr-2">
            {isEmpty(paxMiddleFirstName) ? "--" : paxMiddleFirstName}
          </span>
          <div className="price-config-items">
            {priceConfigItems.map((item, _indexPrc) => (
              <div className="flex" key={_indexPrc}>
                <span className="block w-16 xl:w-24 pr-2">{item.priceConfig.class}</span>
                <span className="block w-20 pr-2">{`${item.quantity} x`}</span>
                <span className="block w-32 pr-2 text-primary-default">
                  {pasengerType ? moneyFormatVND(item.priceConfig[pasengerType]) : null}
                </span>
              </div>
            ))}
          </div>
