...
const DrawerPassengerInfo: React.FC<DrawerPassengerInfoProps> = ({ isOpen, onClose, onSubmit, initialValues }) => {
  ...
  const onSubmitPassengerFormData: HandleSubmit<BookingOrderPassengerFormData> = (data) => {
    onSubmit?.(data);
  };
  /*
   * INITIAL FORM Data
    ...
    >
      ...
      <div className="drawler-action absolute px-4 py-4 border-t left-0 right-0 bg-white bottom-0">
        <Space>
          ...
          <Button onClick={() => handlerSubmit(passengerFormData, onSubmitPassengerFormData)} type="primary">
            Cập nhật
          </Button>
        </Space>
      </div>
