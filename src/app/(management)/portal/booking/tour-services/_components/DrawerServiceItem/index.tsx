...
const DrawerServiceItem: React.FC<DrawerServiceItemProps> = ({
  ...
  const [serviceItems, setServiceItems] = useState<TBookingServicePricingItem[]>([]);
  const [bookingItem, setBookingItem] = useState<IBookingItem>(bookingItems[0]);
  ...
  const onChangeQuantityService = (action: "minus" | "plus", quantity: number, item: PriceConfig) => {
    if (quantity < 0) {
      return;
    }

    const totalAmountOfPricingSvItem = serviceItems.reduce((acc, paxItem) => {
      const pricingItem = paxItem.pricingItems.find((pricingItem) => pricingItem.item.recId === item.recId);
      if (pricingItem) {
        acc += pricingItem.qty;
      }
      return acc;
    ...
    setServiceItems((oldDAta) => {
      ...
      const bookingItemService = newData.find((svItem) => svItem.bookingItem.index === bookingItem.index);
      const bookingItemIndex = newData.findIndex((svItem) => svItem.bookingItem.index === bookingItem.index);

      if (bookingItemIndex !== -1) {
        const pricingIndex = newData[bookingItemIndex].pricingItems.findIndex(
          (pricing) => pricing.item.recId === item.recId,
        );
        let newPricingOfBookingItem = [...newData[bookingItemIndex].pricingItems];
        if (pricingIndex !== -1) {
  ...
  const getQuanityServiceOfPaxByPriceConfig = (item: PriceConfig) => {
    const currentBkServiceItem = serviceItems.find((svItem) => svItem.bookingItem.index === bookingItem.index);

    const pricingItem = currentBkServiceItem?.pricingItems.find((prItem) => prItem.item.recId === item.recId);
  ...
  const subTotal = useMemo(() => {
    return serviceItems.reduce((acc, paxItem) => {
      const totalPricings = paxItem.pricingItems.reduce((subPricing, pricingItem) => {
        subPricing += pricingItem.qty * pricingItem.item[paxItem.bookingItem.type];
        return subPricing;
      }, 0);
      acc += totalPricings;
      return acc;
  ...
  useEffect(() => {
    const servicePaxItems = bookingItems.reduce<TBookingServicePricingItem[]>((acc, bkItem) => {
      /**
       * filter all pricing by sellableId
       */
      const pricingItemsByService = bkItem.ssr.filter((item) => item.sellableDetailsId === sellableDetailsId);

      /**
       * reduce to correct format.
       */

      const pricingItems = pricingItemsByService.reduce<{ qty: number; item: PriceConfig }[]>((accSv, svItem) => {
          ...
          {bookingItems.map((item, _index) => (
            <div
              className={classNames("pax__item p-3 border bg-white shadow-sm rounded-md border-b-2", {
                "border-primary-default": item.index === bookingItem.index,
              })}
              key={_index}
              onClick={() => onSelectPassenger(item)}
            >
              <div className="w-32">
                <span className="text-xs">{`${getPassengerType(item.type)}`}</span>
                <span className="block">{`Hành khách ${_index + 1}`}</span>

                {getTotalQuantityServiceItemOfPax(item) === 0 ? (
                  "--"
                ) : (
            ...
            <div className="pricing__item mb-3" key={_index}>
              <div className="flex justify-between border p-3 shadow-sm rounded-md">
                <div className="flex">
                  ...
                  <div className="w-24">
                    <span className="text-xs block text-gray-500">Đang còn</span>
                    <span>{item.open}</span>
                  </div>
                  <div>
                    <span className="block text-xs">Giá tiền</span>
                    <span className="text-primary-default">{moneyFormatVND(item[bookingItem.type])}</span>
                  </div>
                </div>
                <Quantity
                  size="sm"
                  value={getQuanityServiceOfPaxByPriceConfig(item)}
                  onChange={(action, value) => onChangeQuantityService(action, value, item)}
                />
              </div>
            </div>
          ))}
