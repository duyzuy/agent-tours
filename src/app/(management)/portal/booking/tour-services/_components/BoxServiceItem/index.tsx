...
import DrawerServiceItem, { TBookingServicePricingItem } from "../DrawerServiceItem";
...
export interface BoxServiceItemProps {
  ...
  onSetService: (sellableDetailsId: number, pricingItems: TBookingServicePricingItem[]) => void;
...
const BoxServiceItem: React.FC<BoxServiceItemProps> = ({
  ...
  const pricingItems = useMemo(() => {
    const items = bookingItems.reduce<IBookingItem["ssr"]>((acc, bkItem) => {
      const pricingsItems = bkItem.ssr.filter((svItem) => svItem.sellableDetailsId === serviceItem.sellableDetailsId);
    ...
    const serviceGroupByPaxType = items.reduce<{
      [key: string]: { qty: number; subTotal: number; type: PassengerType } | undefined;
    }>(
      (acc, item) => {
        if (acc[item.type]) {
          acc = {
            ...
            [item.type]: {
              ...
              subTotal: (acc[item.type]?.subTotal || 0) + item.qty * item.item[item.type],
              type: item.type,
            },
          };
        } else {
  ...
  const isSelectedService = useMemo(() => {
    return bookingItems.some((item) =>
      item.ssr.some((ssrItem) => ssrItem.sellableDetailsId === serviceItem.sellableDetailsId),
    );
  ...
  return (
    <>
        ...
        <div className="service__item-inner">
          <div className="service__item-head px-6 py-6 flex justify-between">
            <div className="service-name ">
              <span className="font-[500]">{serviceItem.detail}</span>
            </div>
            <Button type="primary" ghost size="small" onClick={() => setOpenDrawer(true)} className="cursor-pointer">
              Chọn
            </Button>
          </div>
          {render?.()}
              ...
              {Object.keys(pricingItems).map((itemKey) => (
                <React.Fragment key={itemKey}>
                  {pricingItems[itemKey] ? (
                    <div className="pricing-item flex py-1" key={itemKey}>
                      <div className="w-20">{getPassengerType(itemKey as PassengerType)}</div>
                      <div>
                        <span>{`x ${pricingItems[itemKey]?.qty}`}</span>
                      </div>
                      <div className="flex-1 pl-6">
                        <span className="text-primary-default">{moneyFormatVND(pricingItems[itemKey]?.subTotal)}</span>
                      </div>
                    </div>
                  ) : null}
                </React.Fragment>
      ...
      <DrawerServiceItem
        ...
        onConfirm={(pricingsService) => onSetService(serviceItem.sellableDetailsId, pricingsService)}
      />
    </>
  );
};
