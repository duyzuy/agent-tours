...
import { Form, Input, Row, Col, Button, Select, DatePickerProps, SelectProps, Radio, RadioChangeEvent } from "antd";
...
const BoxBooking: React.FC<BoxBookingProps> = ({ className = "searchbox px-4 py-2 bg-white shadow-lg rounded-lg" }) => {
  const { data: destinationList, isLoading: isLoadingDestinationList } = useGetLocalSearchListMISCQuery();
  ...
  const [formData, setFormData] = useState(() => new SearchBookingFormData(undefined, "", [], [EProductType.TOUR], []));
  ...
  const onChangeInventoryType: SelectProps<EInventoryType[]>["onChange"] = (values) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const onChangeDestination: SelectProps<number, LocalSearchDestinationListRs["result"][0]>["onChange"] = (
    value,
    options,
  ) => {
    setFormData((prev) => ({
            ...
            <Row align="bottom" gutter={16}>
              <Col span={6}>
                <FormItem
                  ...
                  help={errors?.byDest || ""}
                >
                  <Select<number, LocalSearchDestinationListRs["result"][0]>
              ...
              <Col flex={1} className="text-right">
                <FormItem>
                  <Button
                    ...
                    icon={<SearchOutlined />}
                    loading={isLoading}
                    onClick={() => handlerSubmit(formData, onSearchBooking)}
                  >
                    Tìm kiếm
                  </Button>
                </FormItem>
