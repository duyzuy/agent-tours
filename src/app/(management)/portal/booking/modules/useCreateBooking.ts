...
import { IBookingItem, IBookingTourPayload } from "./bookingInformation.interface";
...
const useCreateBooking = () => {
  ...
  const getBookingDetailsItems = (items?: IBookingItem[]): IBookingTourPayload["bookingDetails"] => {
    ...
    bookingDetails = items?.reduce<IBookingTourPayload["bookingDetails"]>((acc, bkItem) => {
      const ssrItems = bkItem.ssr.reduce<IBookingTourPayload["bookingDetails"][0]["ssr"]>((acc, ssrItem) => {
        acc = [
          ...acc,
          {
