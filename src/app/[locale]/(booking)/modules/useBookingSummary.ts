...
const useBookingSummary = () => {
    ...
    const { product, bookingSsrWithPax, passengers, couponPolicy, coupons } =
    ...
    const getServiceBreakdownGroupByPassenger = () => {
        return passengers.reduce<BreakDownServiceItemGroupByPassengerType[]>(
            (accBreakdown, paxItem) => {
                const ssrItemsBypax = bookingSsrWithPax?.filter(
                    (item) => item.paxIndex === paxItem.index,
                );

                if (ssrItemsBypax) {
                    let ssrList = ssrItemsBypax.reduce<
                        BreakDownServiceItemGroupByPassengerType["ssrList"]
                    >((acc, item) => {
                        const sellableDetailId = item.sellableDetailId;

                        if (acc[sellableDetailId]) {
                            const configItems =
                                acc[sellableDetailId].priceConfigs;

                            let newConfigItems = [...configItems];

                            const indexConfigItem = configItems.findIndex(
                                (cfItem) =>
                                    cfItem.item.recId ===
                                    item.priceConfig.recId,
                            );
                            ...
                            if (indexConfigItem !== -1) {
                                newConfigItems.splice(indexConfigItem, 1, {
                                    ...
                                    quantity:
                                        configItems[indexConfigItem].quantity +
                                        1,
                                });
                            } else {
                                newConfigItems = [
    ...
    const getServiceBreakdown = () => {
        return bookingSsrWithPax?.reduce<BreakdownServices>(
            (acc, bkSSRItem) => {
                const sellableDetailId = bkSSRItem.sellableDetailId;
                const serviceItem = acc[sellableDetailId];

                const paxItem = passengers.find(
                    (item) => item.index === bkSSRItem.paxIndex,
                );
                if (!paxItem) {
                ...
                if (serviceItem) {
                    ...
                    const _paxIndex = bookingPassengers.findIndex(
                        (pax) => pax.index === bkSSRItem.paxIndex,
                    );

                    if (_paxIndex !== -1) {
                        const bookingpriceConfigs =
                            bookingPassengers[_paxIndex].priceConfigs;
                        let newPriceConfigs = [...bookingpriceConfigs];

                        const _indexPriceConfig = bookingpriceConfigs.findIndex(
                            (cfItem) =>
                                cfItem.priceConfig.recId ===
                                bkSSRItem.priceConfig.recId,
                        );
                    ...
                    acc = {
                        ...
                        [sellableDetailId]: {
                            ...acc[sellableDetailId],
                            subTotal:
                                acc[sellableDetailId].subTotal +
                                bkSSRItem.priceConfig[bkSSRItem.paxType],
