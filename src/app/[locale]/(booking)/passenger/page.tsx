...
const PassengerPage = () => {
  ...
  const [nextSubmitAction, setNextAction] = useState<ESubmitAction>(ESubmitAction.SET_PASSENGER_INFO);

  const { handleSubmit, control, setValue, clearErrors } = useForm<PassengerFormValues>({
  ...
  useEffect(() => {
    isCompletePassengerInformation && setNextAction(ESubmitAction.NEXT_TO_PAYMENT);
  }, []);
    ...
    <>
      <div className="page-passenger rounded-lg">
        ...
        {isCompletePassengerInformation && !canEditPax ? (
          <ServiceContainer
            showService={isCompletePassengerInformation && !canEditPax}
            isCompletedPassengerInfo={isCompletePassengerInformation}
            passengerList={passengers}
