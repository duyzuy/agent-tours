...
import TemplateMetaContentForm, {
    TemplateMetaContentFormProps,
} from "./TemplateMetaContentForm";
import FileDownloadSelector, {
    FileDownloadSelectorProps,
} from "./FilesDownloadSelector";
...
export interface CMSTemplateContentFormProps {
    ...
    onChangeStatus?: (
        id: number,
        type: PageContentStatus.PUBLISH | PageContentStatus.UNPUBLISH,
    ) => void;
}
...
const CMSTemplateContentForm: React.FC<CMSTemplateContentFormProps> = ({
    ...
    const onChangeMetaDataForm: MetaDataFieldsProps["onChange"] = (
        data,
        index,
    ) => {
    ...
    const onSaveFilesDownload: FileDownloadSelectorProps["setFiles"] = (
        files,
    ) => {
        setFormData((oldData) => ({
            ...oldData,
        ...
        setFormData((prev) => {
            return initData
                ? {
                      ...
                      lang: initData.lang,
                      status: isEmpty(initData.status)
                          ? PageContentStatus.PUBLISH
                          : initData.status,
                  }
                : {
