...
import { VisaTemplateContentMetaDataForm } from "../../../modules/visaTemplate.interface";
...
const TemplateMetaContentForm: React.FC<TemplateMetaContentFormProps> = ({
    ...
    const initFormData = new VisaTemplateContentMetaDataForm(
        undefined,
        referenceId,
        "",
        [],
    ...
    const onChangeContentInTab: BlockMetaContentProps["onChange"] = (
        data,
        index,
    ) => {
    ...
    const defaultTabs = [
        {
            ...
            children: (
                <TextEditor
                    onEditorChange={(data, editor) => onChange("content", data)}
    ...
    const tabsItems = useMemo(() => {
        const itemsTab = formData.metaContent.reduce<TabsProps["items"]>(
            (acc, item, _index) => {
                return [
                    ...(acc || []),
                    {
    ...
    const onChange = (
        key: keyof VisaTemplateContentMetaDataForm,
        value: VisaTemplateContentMetaDataForm[keyof VisaTemplateContentMetaDataForm],
    ) => {
        setFormData((prev) => ({
            ...prev,
            [key]: value,
    ...
    const isDisabledButton = useMemo(() => {
        return isEqualObject(
            ["content", "metaContent"],
            formData,
            initialData ? initialData : initFormData,
        );
        ...
        <>
            ...
            <TextEditor
                onEditorChange={(data, editor) =>
                    onChange?.({ title: value?.title, content: data }, index)
                }
                value={value?.content}
            />
        </>
