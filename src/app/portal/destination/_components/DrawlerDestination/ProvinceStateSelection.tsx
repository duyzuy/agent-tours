...
const ProvinceStateSelection: React.FC<ProvinceStateSelectionProps> = ({
    ...
    const changeRegion = (
        ...
        const allChildIdOfRegion = provinceList.reduce<number[]>(
            (acc, item) => {
        ...
        if (!ev.target.checked) {
            ...
            const excerptRestChildsOfRegion = childItemsOfRegion.filter(
                (item) =>
                    ...
                    !allChildIdOfRegion.includes(item.recId),
            );
    ...
    const changeSubRegion = (
        ...
        const currentChildItemsOfSubRegion = provinceList.reduce<number[]>(
            (acc, item) => {
                if (
                    item.regionKey === subRegion.regionKey &&
                    item.subRegionKey === subRegion.subRegionKey
    ...
    const changeCountry = (
        ...
        const currentChildItemsOfCountry = provinceList.reduce<number[]>(
            (acc, item) => {
                if (
                    item.regionKey === country.regionKey &&
                    item.subRegionKey === country.subRegionKey &&
    ...
    const changeProvinceState = (
        ...
        if (!ev.target.checked) {
            ...
            const currentRegionLenghOfState = provinceList.filter(
                (item) => item.regionKey === state.regionKey,
            ).length;
                            ...
                            >
                                <div className="name py-2 font-bold">
                                    <Checkbox
                                        name="sub-region"
                                        onChange={(ev) =>
                                            changeSubRegion(ev, subRegion)
                                        }
                                        checked={hasCheckedState(
                                            subRegion,
                                            "SUBREGIONLIST",
                                        )}
                                        disabled={!editAble}
