...
const useSummaryBookingOrder = () => {
    ...
    const totalAmountNewOrder = useMemo(() => {
        ...
        if (ssrEditting.bookingDetails) {
            const totalAddNew = Object.entries(
                ssrEditting.bookingDetails,
            ).reduce((subtotal, [key, value]) => {
                const totalPaxOfsv = value.items.reduce(
                    (totalAmountItem, curent) => {
                        const totalOfpax = curent.ssr.reduce(
                            (ssrTotal, ssrItem) => {
                                return (ssrTotal +=
                                    ssrItem.priceConfig[ssrItem.type] *
                                    ssrItem.quantity);
                            },
                            0,
                        );
        ...
        const totalAmountRemove = ssrEditting.bookingSsrDelete.reduce(
            (acc, item) => {
    ...
    const groupingServices = useMemo(() => {
        ...
        totalItemsUpdated = orderDetail?.ssr?.reduce<{
            ...
            if (acc[detailsId]) {
                 ...
                 */
                const newItems = bookingSsrDelete.some(
                    (itemDelete) => itemDelete.recId === booking.recId,
                )
                    ? [...acc[detailsId].items]
                    : [
                          ...acc[detailsId].items,
                ...
                const item = bookingSsrDelete.some(
                    (itemDelete) => itemDelete.recId === booking.recId,
                )
                    ? undefined
                    : {
                ...
                if (totalItemsUpdated?.[values.serviceId]) {
                    ...
                    totalItemsUpdated = {
                        ...
                        [values.serviceId]: {
                            ...
                            items: [...listItemNew],
                            serviceName:
                                values.items[0].ssr[0].priceConfig.details,
                        },
                    };
                }
