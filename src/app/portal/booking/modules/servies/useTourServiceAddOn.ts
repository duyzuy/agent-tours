...
const useTourServiceAddOn = () => {
    ...
    const getTotalQuantityOfPricingItemInBooking = (
        recId: number,
        ...
        bookingInfo?.bookingItems?.forEach((bkItem) => {
            const serviceItem = bkItem.ssr.find(
                (ssrItem) =>
                    ssrItem.sellableDetailsId === sellableDetailsId &&
                    ssrItem.priceConfigRecId === recId,
            );
    ...
    const onAddService = (
        ...
        const bookingItem = newBookingItems.find(
            (item) => item.index === bookingIndex,
        );
        if (!bookingItem) {
        ...
        if (action === "plus") {
            const pricingsOfServiceItem =
                serviceList
                    ?.filter(
                        (serviceItem) =>
                            serviceItem.sellableDetailsId === sellableDetailsId,
                    )
                    .sort((a, b) => a.adult - b.adult) || [];

            let outOfStock = true;
            for (let i = 0; i < pricingsOfServiceItem.length; i++) {
                const pricingPickedAmount =
                    getTotalQuantityOfPricingItemInBooking(
                        pricingsOfServiceItem[i].recId,
                        sellableDetailsId,
                    );
                const remainOfItem =
                    pricingsOfServiceItem[i].open - pricingPickedAmount;
                ...
                const indexOfSsr = newSSrOfBookingItem.findIndex(
                    (item) =>
                        item.sellableDetailsId === sellableDetailsId &&
        ...
        if (action === "minus") {
             ...
             * Get last item
             */
            const lastPricingItem =
                servicePricingsInBookingItem[
                    servicePricingsInBookingItem.length - 1
                ];
            const indexOfPricingItem = newSSrOfBookingItem.findIndex(
                (ssrBkItem) =>
                    ssrBkItem.priceConfigRecId ===
                    lastPricingItem.priceConfigRecId,
            );
