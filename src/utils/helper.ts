...
export const objectToQueryString = (obj: { [key: string]: string | number }) => {
  return Object.keys(obj).reduce((acc, key, _index) => {
    return acc.concat(_index === 0 ? `${key}=` : `&${key}=`, obj[key].toString());
  }, "");
};
...
export const vietnameseTonesToUnderscoreKeyname = (str: string) => {
  str = str.replace(/à|á|ạ|ả|ã|â|ầ|ấ|ậ|ẩ|ẫ|ă|ằ|ắ|ặ|ẳ|ẵ/g, "a");
  str = str.replace(/è|é|ẹ|ẻ|ẽ|ê|ề|ế|ệ|ể|ễ/g, "e");
  str = str.replace(/ì|í|ị|ỉ|ĩ/g, "i");
  str = str.replace(/ò|ó|ọ|ỏ|õ|ô|ồ|ố|ộ|ổ|ỗ|ơ|ờ|ớ|ợ|ở|ỡ/g, "o");
...
export const getFeeFromShortCode = ({ str, key }: { str: string; key: string }) => {
  // const reg = /(max_fee|max_fee|percent\=\"[0-9]*\")/;
  const reg = new RegExp(`(${key}\=\"[0-9]*\")`, "i");
  const stringData = str.match(reg);

  if (stringData) {
    const val = stringData[0].substring(stringData[0].indexOf('"') + 1, stringData[0].lastIndexOf('"'));
...
export const stringSlashToCamelCase = (str: string) =>
  str.split("_").reduce((acc, word, _index) => {
    const firstChar = _index === 0 ? word.charAt(0).toLowerCase() : word.charAt(0).toUpperCase();
    const rest = word.slice(1);

    acc = acc.concat(firstChar, rest);
    return acc;
  }, "");

export const generateStrings = (length = 8) => {
  let output = "";
