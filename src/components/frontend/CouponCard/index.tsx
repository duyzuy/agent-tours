...
const CouponCard: React.FC<CouponCardProps> = ({
  ...
  return (
    <div
      className={classNames("promocode-item border rounded-md bg-white relative overflow-hidden", {
        [className]: className,
      ...
      <div className="promocode-item__inner relative flex">
        <div className="promocode-item__left w-24 flex items-center justify-center border-r border-dashed mr-3 px-3 py-2 bg-slate-50 rounded-md">
          <span className="text-base -rotate-90 text-center whitespace-nowrap leading-none">
            <span className="block text-[10px] text-gray-500 uppercase">{t("promocodeItem.code")}</span>
            <span>{code}</span>
          </span>
        </div>
        <div
        ...
        >
          <div className="promocode-item__head flex-1 pr-3">
            <div className="promocode-item__price">
              <span className="mr-2 w-24 text-gray-500 text-xs block">{t("promocodeItem.discountValue.label")}</span>
              <span className="text-red-600 font-[500]">{price}</span>
            </div>
            <div className="promocode-item__valid-date">
              <span className="mr-2 w-24 text-gray-500 text-xs block">{t("promocodeItem.expiredDate.label")}</span>
              <span className="promocode-item__valid-date-value">{`${validFrom} - ${validTo}`}</span>
            </div>
          </div>
          <div className="promocode-item__actions">
            <span
              className={classNames(
                ...
                {
                  "bg-emerald-400 border-emerald-400": isSelecting,
                },
              )}
            >
              {isSelecting ? <IconCheck width={16} height={16} /> : null}
            </span>
          </div>
        </div>
      </div>
