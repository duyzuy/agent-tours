...
import { ICON_LABEL_HOTDEAL_LIST } from "@/constants/icons.constant";

type IconType = (typeof ICON_LABEL_HOTDEAL_LIST)[0];
...
const LabelSelector: React.FC<LabelSelectorProps> = ({ value, placeholder = "Chọn icon", label, onChange }) => {
  ...
  const selectedItem = ICON_LABEL_HOTDEAL_LIST.find((item) => item.key === value);
        ...
        <div className="icon-selector__dropdown w-full absolute z-40">
          <div className="icon-selector__dropdown--inner shadow-md rounded-md bg-white px-2 py-4 w-full max-h-[280px] overflow-y-auto">
            <div className="icon-selector__dropdown--list">
              {ICON_LABEL_HOTDEAL_LIST.map((item, _index) => (
                <div
