...
const addResourcesToCache = async (type, resources) => {
  const cache = await caches.open(type === "assets" ? assetsCacheName : imagesCacheName);
  await cache
    .addAll(resources)
    .then(() => {
...
const putInCache = async (type, request, response) => {
  const cache = await caches.open(
    type === "assets" ? assetsCacheName : type === "image" ? imagesCacheName : dynamicCacheName,
  );
...
const deleteOldCaches = async () => {
  ...
  const cachesToDelete = keyList.filter(
    (key) => key !== assetsCacheName && key !== imagesCacheName && key !== dynamicCacheName,
  );
